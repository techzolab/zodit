...
describe('Test object observer', function () {
	...
	const get = Zodit.modules.Helpers.get;
	const stringify = Zodit.modules.Helpers.stringify;
	const isEqual = Zodit.modules.Helpers.isEqual;

	const A = function (result, keyA, keyB) {
		function A() {
			this.zodit = getZodit();
			this.async = this.zodit.async;
		...
		__.prototype = Zodit.modules.ViewComponent.prototype;
		A.prototype = new __();
		...
		describe('Compose with `wait`', () => {
			it('Should work correct', function (done) {
				...
				decorate(
					[
						Zodit.decorators.debounce(100),
						Zodit.decorators.wait(ctx => ctx.someFlag > 2)
					],
					AClass.prototype,
					'callTime'
				);
	...
	describe('Test watch decorator', function () {
		...
		it('Should add watcher to some field in Component', function () {
			...
			decorate(
				[Zodit.decorators.watch('state.some.element.enable')],
				AClass.prototype,
				'methodA'
			);
		...
		describe('Add several watchers', function () {
			...
			describe('on different fields', function () {
				it('Should call only matched handlers', function () {
					...
					decorate(
						[Zodit.decorators.watch('state.some.element.one')],
						AClass.prototype,
						'methodA'
					);

					decorate(
						[Zodit.decorators.watch('state.some.element.two')],
						AClass.prototype,
						'methodB'
					);
		...
		describe('On change field', function () {
			it('Should fire change all parent field', function () {
				...
				decorate(
					[Zodit.decorators.watch('state.some')],
					AClass.prototype,
					'methodA'
				);
	...
	describe('Test object properties', function () {
		describe('Observed object', function () {
			it('Should has only own object properties', function () {
				const a = { a: 1, b: 2 };
				const observed = Zodit.modules.observable(a);
	...
	describe('Event on change', function () {
		it('Should fire event when field value was changed', function () {
			...
			const data = Zodit.modules.observable(getTestObject());
		...
		describe('Change watched property', function () {
			it('Should fire handler', function () {
				...
				Zodit.decorators.watch('mode')(obj, 'methodA');
