 ...
 * ```js
 * const f = Math.random();
 * Zodit.modules.Helpers.call(f > 0.5 ? Math.ceil : Math.floor, f);
 * ```
 */
 ...
 */
export function markOwner(zodit: IViewBased, elm: HTMLElement): void {
	attr(elm, 'data-editor_id', zodit.id);

	!elm.component &&
		Object.defineProperty(elm, 'zodit', {
			value: zodit
		});
}
...
export const loadImage = (
	...
	zodit.async.promise<HTMLImageElement>((res, rej) => {
 ...
 */
export const memorizeExec = <T extends IZodit = IZodit>(
	editor: T,
	_: unknown,
