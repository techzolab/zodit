...
import type { IBound, IHasScroll, IZodit, IViewBased } from 'zodit/types';
...
export const offset = (
	...
	const iframe = (zodit as IZodit).iframe;

	if (!recurse && zodit && zodit.options && zodit.o.iframe && iframe) {
		const { top, left } = offset(iframe, zodit, zodit.od, true);
