...
describe('Selection Module Tests', function () {
				...
				describe('Cursor in the end of text node but after this has several not empty text nodes', function () {
					...
					describe('Cursor in the end of text node and after are only text nodes with invisible spaces', function () {
						it('Should return true', function () {
							...
							editor.s.insertNode(
								editor.createInside.text(Zodit.INVISIBLE_SPACE)
							);
							editor.s.insertNode(
								editor.createInside.text(Zodit.INVISIBLE_SPACE)
							);
	...
	describe('Change mode', function () {
		...
		it('Should restore collapsed selection when user change mode - from WYSIWYG to TEXTAREA for long string', function (done) {
			...
			Zodit.make(appendTestArea(), {
				defaultMode: Zodit.MODE_SOURCE,
				sourceEditor: 'ace',
				beautifyHTML: false,
				events: {
					sourceEditorReady: function (zodit) {
						zodit.setMode(Zodit.MODE_WYSIWYG);
						zodit.setEditorValue(
							('<p>' + 'test '.repeat(50) + '</p>').repeat(1)
						);

						const sel = zodit.ew.getSelection(),
							range = zodit.ed.createRange();

						range.selectNodeContents(
							zodit.editor.querySelector('p')
						);
						...
						zodit.s.insertHTML('hello');

						zodit.setMode(Zodit.MODE_SOURCE);

						const ace =
							zodit.__plugins.source.sourceEditor.instance;
						...
						expect(
							zodit.__plugins.source.sourceEditor.getValue()
						).equals(
	...
	describe('Method eachSelection', function () {
		...
		it('Should call callback for each node in selection range', function () {
			const editor = getZodit({ disablePlugins: ['WrapNodes'] });
			editor.value =
				'<p>1</p>|<p>2</p><strong><span>22</span></strong><p>4</p>|stop';
		...
		[
			...
			describe(`For index ${i}  source: ${source}`, () => {
				it('Should move cursor selection', () => {
					...
					zodit.s.expandSelection();
					replaceCursorToChar(zodit);
					expect(zodit.value).eq(result);
				});
			});
		});
	});
