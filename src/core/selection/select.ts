...
import { moveTheNodeAlongTheEdgeOutward } from 'zodit/core/selection/helpers';

export class Select implements ISelect {
	constructor(readonly zodit: IZodit) {
		zodit.e.on('removeMarkers', () => {
			this.removeMarkers();
		});
	}
	...
	/**
	 * Return current work place - for Zodit is Editor
	 */
	 ...
	 * console.log(Zodit.modules.Helpers.stripTags(editor.s.html)); // plain text
	 * ```
	 */
	get html(): string {
		const sel = this.sel;
	...
	*wrapInTagGen(): Generator<HTMLElement> {
		if (this.isCollapsed()) {
			const font = this.zodit.createInside.element(
				'font',
				INVISIBLE_SPACE
			);
		...
		// fix issue https://github.com/techzolab/zodit/issues/65
