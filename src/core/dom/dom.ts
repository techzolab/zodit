...
import { Select } from 'zodit/core/selection';
import { TEMP_ATTR } from 'zodit/core/constants';
...
export class Dom {
	 ...
	 * @example
	 * ```javascript
	 * Zodit.modules.Dom.replace(parent.editor.getElementsByTagName('span')[0], 'p');
	 ...
	 * @example
	 * ```javascript
	 * Zodit.modules.Dom.each(editor.s.current(), function (node) {
	 *  if (node.nodeType === Node.TEXT_NODE) {
	 *      node.nodeValue = node.nodeValue.replace(Zodit.INVISIBLE_SPACE_REG_EX, '') // remove all of the text element codes invisible character
	 *  }
	 * });
	 * ```
	 */
