...
export class Config implements IViewOptions {
	...
	/**
	 * Shadow root if Zodit was created in it
	 */
	shadowRoot: Nullable<ShadowRoot> = null;

	/**
	 * Dictionary of variable values in css, a complete list can be found here
	 * https://github.com/techzolab/zodit/blob/master/src/styles/variables.less#L25
	 *
	 * @example
	 * ```js
	 ...
	 * @example
	 * ```javascript
	 * var editor  = new  Zodit(".dark_editor", {
	 ...
	 *
	 * @see {@link Zodit.defaultOptions.iframeCSSLinks|iframeCSSLinks}
	 * @see {@link Zodit.defaultOptions.iframeStyle|iframeStyle}
	 *
	 * @example
	 * ```javascript
	 * Zodit.make('#editor', {
	 ...
	 * <script src="zodit/lang/de.js"></script>
	 * <script>
	 * var editor = Zodit.make('.editor', {
	 *    language: 'de'
	 * });
	...
	/**
	 * Zodit.MODE_WYSIWYG The HTML editor allows you to write like MSWord,
	 * Zodit.MODE_SOURCE syntax highlighting source editor
	 * @example
	 * ```javascript
	 * var editor = Zodit.make('#editor', {
	...
	/**
	 * By default, you can only install an icon from the Zodit suite.
	 * You can add your icon to the set using the `Zodit.modules.Icon.set (name, svg Code)` method.
	 * But for a declarative declaration, you can use this option.
	 *
	 * @example
	 * ```js
	 * Zodit.modules.Icon.set('someIcon', '<svg><path.../></svg>');
	 ...
	 * @example
	 * ```js
	 * const editor2 = Zodit.make('#editor', {
	 * 	createAttributes: {
	 * 		div: {
	 * 			class: 'test'
	 * 		},
	 ...
	 * import ZoditEditor from "zodit-react";
	 *
	 * const config = {
	 * 	createAttributes: {
	 * 		div: {
