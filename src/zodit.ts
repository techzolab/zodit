...
import { lang } from 'zodit/core/constants';
import { instances, pluginSystem, modules } from './core/global';
import { autobind, cache, throttle, watch } from './core/decorators';

const __defaultStyleDisplayKey = 'data-zodit-default-style-display';
const __defaultClassesKey = 'data-zodit-default-classes';

/**
 * Class Zodit. Main class
 */
export class Zodit extends ViewWithToolbar implements IZodit {
	/** @override */
	 ...
	 * await zodit.waitForReady();
	 * zodit.e.fire('someAsyncLoadedPluginEvent', (test) => {
	 *   alert(test);
	 * });
	 * ```
	 */
	waitForReady(): Promise<IZodit> {
		if (this.isReady) {
	...
	/**
	 * Define if object is Zodit
	 */
	override readonly isZodit: true = true;
	...
	/**
	 * Factory for creating Zodit instance
	 */
	static make(element: HTMLElement | string, options?: object): Zodit {
		return new Zodit(element, options);
	}

	/**
	 * Checks if the element has already been initialized when for Zodit
	 */
	static isZoditAssigned(
		element: HTMLElement
	): element is HTMLElement & { component: Zodit } {
		return (
			element &&
			isZoditObject(element.component) &&
	...
	static readonly instances: IDictionary<IZodit> = instances;
	...
	private readonly commands: Map<string, Array<CustomCommand<IZodit>>> =
		new Map();
	...
	/**
	 * @deprecated Instead use `Zodit.history`
	 */
	...
	/**
	 * options All Zodit settings default + second arguments of constructor
	 */
	...
	get filebrowser(): IFileBrowser {
		...
		const options = ConfigProto(
			{
				defaultTimeout: zodit.defaultTimeout,
				uploader: zodit.o.uploader,
				language: zodit.o.language,
				license: zodit.o.license,
				theme: zodit.o.theme,
						...
						data.files.forEach((file, i) => {
							...
							if (isImage) {
								zodit.s.insertImage(
									url,
									null,
									zodit.o.imageDefaultWidth
								);
							} else {
								zodit.s.insertNode(
									zodit.createInside.fromHTML(
		...
		return zodit.getInstance<IFileBrowser>('FileBrowser', options);
	}
	...
	getEditorValue(
		 ...
		 * @example
		 * ```javascript
		 * var editor = Zodit.make("#redactor");
		...
		if (removeSelectionMarkers) {
			value = value.replace(
				/<span[^>]+id="zodit-selection_marker_[^>]+><\/span>/g,
				''
			);
		}
	...
	/**
	 * @deprecated Use `Zodit.value` instead
	 */
	 ...
	 * @example
	 * ```javascript
	 * var zodit = Zodit.make('#editor);
	 *
	 * zodit.setEditorValue('test test test');
	 *
	 * zodit.registerCommand('replaceString', function (command, needle, replace) {
	 *      var value = this.getEditorValue();
	 *      this.setEditorValue(value.replace(needle, replace));
	 *      return false; // stop execute native command
	 * });
	 *
	 * zodit.execCommand('replaceString', 'test', 'stop');
	 *
	 * console.log(zodit.value); // stop test test
	 *
	 * // and you can add hotkeys for command
	 * zodit.registerCommand('replaceString', {
	 *    hotkeys: 'ctrl+r',
	...
	private execCustomCommands(
		...
		if (commands !== undefined) {
			...
			commands.forEach((command: CustomCommand<Zodit>) => {
				let callback: ExecCommandCallback<Zodit>;
	...
	override lock(name = 'any'): boolean {
		if (super.lock(name)) {
			...
			this.s.clear();
			this.editor.classList.add('zodit_lock');
	...
	override unlock(): boolean {
		if (super.unlock()) {
			this.editor.classList.remove('zodit_lock');
	...
	/**
	 * Return current editor mode: Zodit.MODE_WYSIWYG, Zodit.MODE_SOURCE or Zodit.MODE_SPLIT
	 */
	getMode(): Modes {
		return this.mode;
	}
	...
	setMode(mode: number | string): void {
		 ...
		 *     data.mode = Zodit.MODE_SOURCE; // not respond to the mode change. Always make the source code mode
		 * });
		 * ```
		 */
		...
		if (this.o.saveModeInStorage) {
			this.storage.set('zodit_default_mode', this.mode);
		}
	...
	setDisabled(isDisabled: boolean): void {
		...
		if (this.editor) {
			...
			this.container.classList.toggle('zodit_disabled', isDisabled);
	...
	constructor(element: HTMLElement | string, options?: object) {
		...
		try {
			...
			if (Zodit.isZoditAssigned(elementSource)) {
				// @ts-ignore
	...
	addPlace(
		...
		let container = this.c.div('zodit-container');

		container.classList.add('zodit');
		container.classList.add('zodit-container');
		container.classList.add(`zodit_theme_${this.o.theme || 'default'}`);
		...
		if (this.o.inline) {
			...
			container.classList.add('zodit_inline');
			container.classList.add('zodit-container');
		}
		...
		const workplace = this.c.div('zodit-workplace', {
		...
		const editor = this.c.div('zodit-wysiwyg', {
	...
	private initEditor(buffer: null | string): void | Promise<any> {
		...
		return callPromise(result, () => {
			...
			if (this.o.saveModeInStorage) {
				const localMode = this.storage.get('zodit_default_mode');
	...
	/**
	 * Zodit's Destructor. Remove editor, and return source input
	 */
