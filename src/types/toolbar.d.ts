...
interface IControlType<
	T = IZodit | IViewBased | IFileBrowser,
	B = IToolbarButton
> {
	name?: string;
	text?: string;
	 ...
	 * @example
	 * ```javascript
	 * var editor = Zodit.make('.selectorclass', {
	 *     buttons: {
	 *          checkbox: {
	 *              data: {
	 *                  active: false,
	 ...
	 * (or array value) (see .[[Zodit.execCommand]] or define 'exec' function. See example
	 * @example
	 * ```javascript
	 * Zodit.make('#editor2', {
	 ...
	 * and several specific [[Zodit.execCommand]]
	 */
	command?: string;
	tagRegExp?: RegExp;
	 ...
	 */
	template?: (zodit: T, key: string, value: string) => string;
	childTemplate?: (zodit: T, key: string, value: string, button: IToolbarButton) => string;
