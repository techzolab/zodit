...
import type { IZodit, ISourceEditor } from 'zodit/types';
...
import { isString, loadNext } from 'zodit/core/helpers';
...
export class source extends Plugin {
	...
	private tempMarkerStart = '{start-zodit-selection}';
	private tempMarkerStartReg = /{start-zodit-selection}/g;
	private tempMarkerEnd = '{end-zodit-selection}';
	private tempMarkerEndReg = /{end-zodit-selection}/g;
	...
	protected removeSelection(): void {
		...
		try {
			value = value
				.replace(
					/<span[^>]+data-zodit-selection_marker=(["'])start\1[^>]*>[<>]*?<\/span>/gim,
					this.tempMarkerStart
				)
				.replace(
					/<span[^>]+data-zodit-selection_marker=(["'])end\1[^>]*>[<>]*?<\/span>/gim,
					this.tempMarkerEnd
				);
	...
	/** @override */
	afterInit(editor: IZodit): void {
		this.mirrorContainer = editor.c.div('zodit-source');
	...
	private initSourceEditor(editor: IZodit): void {
