...
import type { IZodit, MarkerInfo, Nullable } from 'zodit/types';
import { Icon, UIElement } from 'zodit/core/ui';
import { css, position, refs, trim } from 'zodit/core/helpers';
import { MODE_WYSIWYG } from 'zodit/core/constants';
...
@component
export class UISearch extends UIElement<IZodit> {
		...
		return `<div>
			<div class="&__box">
				...
				<div class="&__buttons">
					...
					<button data-ref="replace-btn" tabindex="0" type="button" class="zodit-ui-button">~Replace~</button>
				</div>
			</div>
		</div>`;
	}
		...
		zodit.e
			...
			.on(this.queryInput, 'pointerdown', () => {
				if (zodit.s.isFocused()) {
					...
					this.selInfo = zodit.s.save();
				}
			})
			.on(this.replaceButton, 'pointerdown', () => {
				zodit.e.fire(this, 'pressReplaceButton');
				return false;
			})
			...
			.on(
				...
				this.j.async.debounce((e: KeyboardEvent) => {
					switch (e.key) {
						...
						default:
							zodit.e.fire(this, 'needUpdateCounters');
							break;
					}
