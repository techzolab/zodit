...
describe('Test plugins', function () {
	...
	describe('Add new Line plugin', function () {
		it('Should not add new line element in container before first use', function () {
			const editor = getZodit();
			expect(
				editor.container.querySelectorAll('.zodit-add-new-line').length
			).equals(0);
		});

		const moveCursorUnder = function (editor, elm) {
			simulateEvent('mousemove', 0, editor.editor, function (e) {
				const pos = Zodit.modules.Helpers.position(elm, editor);
		...
		it('Should show .zodit-add-new-line after user move mouse under Table,Ifrmae or IMG ', function () {
			const editor = getZodit();
			editor.value =
				'<table>' +
				'<tbody>' +
		...
		it('Should add new paragraph after user clicked on newline below table', function () {
			...
			simulateEvent('mousemove', 0, editor.editor, function (data) {
				const pos = Zodit.modules.Helpers.position(
				...
				describe('Enable ratio', function () {
					it('Should deny crop image without ratio', function (done) {
						...
						const editor = Zodit.make(area, {
							...
							uploader: {
								url: 'https://techzolab.net/zodit/connector/index.php?action=upload'
							},
							filebrowser: {
								ajax: {
						...
						editor.value =
							'<img alt="" src="https://techzolab.net/zodit/files/th.jpg">';
						...
						editor.filebrowser.events.on(
							...
							function () {
								...
								expect(
									imageEditor.querySelectorAll(
										'[data-area=crop].zodit-image-editor_active'
									).length
								).equals(0);
								...
								simulateEvent(
									'mousedown',
									0,
									cropper.querySelector('.zodit_bottomright'),
									function (e) {
										const pos =
											Zodit.modules.Helpers.offset(
												cropper,
						...
						editor.filebrowser.events.on(
							...
							function () {
								...
								const disableRatioBtn = imageEditor
									...
									.querySelector('.zodit-switcher input');
					...
					it('Should deny resize image without ratio', function (done) {
						...
						editor.filebrowser.events.on(
							...
							function () {
								...
								expect(
									imageEditor.querySelectorAll(
										'[data-area=resize].zodit-image-editor_active'
									).length
								...
								simulateEvent(
									'mousedown',
									0,
									resizer.querySelector('.zodit_bottomright'),
									function (e) {
										const pos =
											Zodit.modules.Helpers.offset(
												resizer,
		...
		describe('Symbols dialog', function () {
			...
			describe('Press Enter or mousdown on element', function () {
				it('Should insert character', function () {
					...
					simulateEvent('keydown', Zodit.KEY_ENTER, currentActive);
	...
	describe('Sticky plugin', function () {
		describe('Without scrolling', function () {
			it('Should not have `zodit_sticky` class and toolbar must be in normal state', function () {
				const area = appendTestArea(),
					editor = new Zodit(area);

				editor.value = '<p>stop</p>'.repeat(100);
				expect(false).equals(
					editor.container.classList.contains('zodit_sticky')
				);
			});
		});
		...
		describe('Create editor in page with long text', function () {
			describe('and scroll page to bottom', function () {
				it('Should add to editor class `zodit_sticky` and toolbar must be always on the top', function () {
					const editor = getZodit();

					editor.value = '<p>stop</p>'.repeat(100);

					const offset = Zodit.modules.Helpers.offset(
						editor.container,
						editor,
						editor.ownerDocument
					);
				...
				describe('On mobile devices - with toolbarDisableStickyForMobile = true', function () {
					it('Should not add to editor class `zodit_sticky`', function () {
			...
			describe('and scroll page to the top', function () {
				it('Should remove class `zodit_sticky` from editor and toolbar must have normal position', function () {
					fillBoxBr(100);
					...
					expect(5).to.be.above(
						Math.abs(
							200 -
								Zodit.modules.Helpers.position(
		...
		describe('After init', function () {
			describe('With showXPathInStatusbar=true', function () {
				it('Should show status bar', function () {
					...
					const statusbar =
						editor.container.querySelector('.zodit-status-bar');
				...
				describe('After click on element of path', function () {
					it('Should select this element', function () {
						...
						expect(
							Zodit.modules.Helpers.trim(editor.s.sel.toString())
						).equals('sss');
						expect(statusbar.childNodes[2].textContent).equals('a');
				...
				describe('Context menu on element of path', function () {
					it('Should open context menu', function () {
						...
						const elm = statusbar.childNodes[2].querySelector('a'),
							pos = Zodit.modules.Helpers.position(elm);
			...
			describe('After click on some of elements', function () {
				it('Sholud select this', function () {
					...
					simulateEvent(
						'click',
						0,
						dialog.querySelectorAll('.zodit-paste-storage a')[1]
					);

					expect(
						dialog
							.querySelectorAll('.zodit-paste-storage a')[1]
							.classList.contains('zodit_active')
					).is.true;
			...
			describe('Press key up/down/enter', function () {
				it('Sholud select next/previos element of list and insert selected value after Enter', function () {
					...
					simulateEvent(
						'click',
						0,
						dialog.querySelectorAll('.zodit-paste-storage a')[0]
					);
					expect(
						dialog
							.querySelectorAll('.zodit-paste-storage a')[0]
							.classList.contains('zodit_active')
					).is.true;
