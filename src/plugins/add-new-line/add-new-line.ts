...
import type { IBound, IZodit, HTMLTagNames, Nullable } from 'zodit/types';
import { Dom, Icon, Plugin } from 'zodit/modules';
import {
	offset,
	position,
	call,
...
/**
 * Create helper for adding new paragraph(Zodit.defaultOptions.enter tag) before iframe, table or image
 */
export class addNewLine extends Plugin {
	private line = this.j.c.fromHTML(
		`<div role="button" tabindex="-1" title="${this.j.i18n(
			'Break'
		)}" class="zodit-add-new-line"><span>${Icon.get('enter')}</span></div>`
	) as HTMLDivElement;
	...
	private show(): void {
		...
		this.line.classList.toggle('zodit-add-new-line_after', !this.preview);
