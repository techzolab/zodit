...
import { css, attr } from 'zodit/core/helpers';
import { Dom } from 'zodit/core/dom';
import { Plugin } from 'zodit/core/plugin';
import { MAY_BE_REMOVED_WITH_KEY } from 'zodit/core/constants';
...
export class placeholder extends Plugin {
	...
	protected afterInit(editor: IZodit): void {
		...
		this.placeholderElm = editor.c.fromHTML(
			`<span data-ref="placeholder" style="display: none;" class="zodit-placeholder">${editor.i18n(
	...
	protected beforeDestruct(zodit: IZodit): void {
		...
		zodit.e.off('.placeholder').off(window, 'load', this.toggle);
	}
}
