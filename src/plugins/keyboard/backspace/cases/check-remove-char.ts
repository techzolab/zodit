...
import type { CanUndef, IZodit, Nullable } from 'zodit/types';
import type { DeleteMode } from 'zodit/plugins/keyboard/backspace/interface';
import { findMostNestedNeighbor } from 'zodit/plugins/keyboard/helpers';
import { Dom } from 'zodit/core/dom';
import { call, isVoid, toArray, trimInv } from 'zodit/core/helpers';
import { INVISIBLE_SPACE, NBSP_SPACE } from 'zodit/core/constants';
...
export function checkRemoveChar(
	...
	while (sibling && (Dom.isText(sibling) || Dom.isInlineBlock(sibling))) {
		...
		if (!isVoid(removed) && removed !== INVISIBLE_SPACE) {
			...
			if (
				...
				checkRemoveChar(zodit, fakeNode, backspace, mode);
			}
		...
		if (
			!nextSibling &&
			sibling.parentNode &&
			sibling.parentNode !== zodit.editor
		) {
	...
	if (charRemoved) {
		...
		addBRInsideEmptyBlock(zodit, fakeNode);
 ...
 */
function addBRInsideEmptyBlock(zodit: IZodit, node: Node): void {
	if (
		node.parentElement !== zodit.editor &&
		Dom.isBlock(node.parentElement) &&
		Dom.each(node.parentElement, Dom.isEmptyTextNode)
	) {
		Dom.after(node, zodit.createInside.element('br'));
	}
}
