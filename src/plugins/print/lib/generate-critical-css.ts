...
import type { IZodit, IDictionary } from 'zodit/types';
import { toArray } from 'zodit/core/helpers/array/to-array';

export function generateCriticalCSS(zodit: IZodit): string {
	...
	try {
		const cp = new CSSCriticalPath(zodit.ew, zodit.ed, {
			scanFullPage: true
		});
