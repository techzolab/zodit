...
import type { HTMLTagNames, IBound, Nullable } from 'zodit/types';
import type { IZodit } from 'zodit/types';
import * as consts from 'zodit/core/constants';
import { IS_IE, KEY_ALT } from 'zodit/core/constants';
...
import { eventEmitter } from 'zodit/core/global';
import { autobind, debounce, watch } from 'zodit/core/decorators';
import './config';

const keyBInd = '__zodit-resizer_binded';
...
export class resizer extends Plugin {
	...
	private rect = this.j.c.fromHTML(
		`<div title="${this.j.i18n(
			'Press Alt for custom resizing'
				...
				<div class="zodit-resizer__top-left"></div>
				<div class="zodit-resizer__top-right"></div>
				<div class="zodit-resizer__bottom-right"></div>
				<div class="zodit-resizer__bottom-left"></div>
				<span>100x100</span>
			</div>`
	);
	...
	protected afterInit(editor: IZodit): void {
		...
		editor.e
			...
			.on(
				...
				(data: { value: string }) => {
					const rgx =
						/<zodit[^>]+data-zodit_iframe_wrapper[^>]+>(.*?<iframe[^>]*>.*?<\/iframe>.*?)<\/zodit>/gi;
	...
	private bind(element: HTMLElement): void {
		...
		if (Dom.isTag(element, 'iframe')) {
			...
			if (
				...
				attr(element.parentNode, '-zodit_iframe_wrapper')
			) {
				element = element.parentNode;
			} else {
				wrapper = this.j.createInside.element('zodit', {
					'data-zodit-temp': 1,
					contenteditable: false,
					draggable: true,
					'data-zodit_iframe_wrapper': 1
				});
				...
				this.j.e.on(wrapper, 'click', () => {
					attr(wrapper, 'data-zodit-wrapper_active', true);
				});
	...
	private hide(): void {
		if (!this.isResizeMode) {
			...
			$$("[data-zodit-wrapper_active='true']", this.j.editor).forEach(
				elm => attr(elm, 'data-zodit-wrapper_active', false)
			);
		}
	}
	...
	protected beforeDestruct(zodit: IZodit): void {
		...
		zodit.e.off(this.j.ow, '.resizer').off('.resizer');
	}
}
