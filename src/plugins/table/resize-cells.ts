...
export class resizeCells extends Plugin {
	...
	private createResizeHandle = (): void => {
		if (!this.resizeHandler) {
			this.resizeHandler = this.j.c.div('zodit-table-resizer');
	...
	private onHandleMouseDown(event: MouseEvent): boolean | void {
		...
		this.j.lock(key);
		this.resizeHandler.classList.add('zodit-table-resizer_moved');
	...
	private onMouseUp(e: MouseEvent): void {
		...
		this.resizeHandler.classList.remove('zodit-table-resizer_moved');
	...
	beforeDestruct(zodit: IZodit): void {
		if (zodit.events) {
			zodit.e.off(this.j.ow, '.resize-cells');
			zodit.e.off('.resize-cells');
		}
	}
}
