...
import { css, defaultLanguage, attr, callPromise } from 'zodit/core/helpers/';
import { error } from 'zodit/core/helpers';
import { MODE_SOURCE } from 'zodit/core/constants';
 ...
 */
export function iframe(editor: IZodit): void {
	...
	editor.e
		...
		.on(
			...
			(__doc: Document | undefined, zodit: IZodit) => {
				const doc =
					__doc ||
					(
						(zodit.iframe as HTMLIFrameElement)
				...
				doc.write(
					opt.iframeDoctype +
						`<html dir="${
							opt.direction
						}" class="zodit" lang="${defaultLanguage(
							opt.language
						)}">` +
						'<head>' +
		...
		.on('createEditor', (): void | Promise<void> | false => {
			...
			iframe.className = 'zodit-wysiwyg_iframe';
						...
						html = html
							...
							.replace(
								...
								body[0]
									...
									.replace(
										/<(style|script|span)[^>]+zodit[^>]+>.*?<\/\1>/g,
										''
									)
							)
							.replace(
								/(class\s*=\s*)(['"])([^"']*)(zodit-wysiwyg|zodit)([^"']*\2)/g,
								'$1$2$3$5'
							)
