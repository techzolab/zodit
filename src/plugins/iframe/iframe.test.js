			...
			describe('With AREA source element', function () {
				...
				describe('editor.value', function () {
					it('Should return entire HTML', function () {
						const editor = Zodit.make(appendTestArea(), opt);
			...
			Zodit.make(area, {
				...
				events: {
					afterConstructor: function (zodit) {
						expect(
							zodit.editor.getAttribute('secret-attribute')
						).equals('435'); // loaded from index.html
						expect(Zodit.ns.Helpers.trim(zodit.value)).equals(
							'<p>test 435</p>'
					...
					['generateDocumentStructure.iframe']: function (
						...
						zodit.events.stopPropagation(
							'generateDocumentStructure.iframe'
						);
						return new Promise(resolve => {
							zodit.iframe.onload = function () {
								resolve();
							};
							...
							zodit.iframe.src = 'test.index.html';
						});
					}
				}
			});
