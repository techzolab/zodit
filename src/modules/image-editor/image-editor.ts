...
import { ViewComponent } from 'zodit/core/component';
import { Alert, Dialog, Prompt } from 'zodit/modules/dialog';
import { $$, attr, call, css, refs, toArray, trim } from 'zodit/core/helpers';
import { Dom } from 'zodit/core/dom';
import { Button } from 'zodit/core/ui/button';
import { form } from './templates/form';
import { component, debounce, throttle, autobind } from 'zodit/core/decorators';
import './config';
...
export class ImageEditor extends ViewComponent {
	...
	private setHandlers = (): void => {
		...
		self.j.e
			.on(
				[
					self.editor.querySelector('.zodit_bottomright'),
					self.cropHandler
				],
				`mousedown.${jie}`,
	 ...
	 *		 }
	 * });
	 * zodit.imageeditor.open('http://techzolab.net/opensource/zodit/images/test.png', function (name, data, success, failed) {
	 *		 var img = zodit.node.c('img');
	 *		 img.setAttribute('src', 'http://techzolab.net/opensource/zodit/images/test.png');
	...
	open(url: string, save: onSave): Promise<Dialog> {
		return this.j.async.promise<Dialog>((resolve: Function): void => {
			...
			$$('img,.zodit-icon_loader', this.resize_box).forEach(
				Dom.safeRemove
			);

			$$('img,.zodit-icon_loader', this.crop_box).forEach(Dom.safeRemove);
			...
			this.resize_box.appendChild(
				this.j.c.element('i', { class: 'zodit-icon_loader' })
			);
	...
	constructor(editor: IViewBased) {
		...
		this.options =
			editor && (editor as IZodit).o && (editor as IZodit).o.imageeditor
				? (editor as IZodit).o.imageeditor
		...
		this.sizes = this.editor.querySelector(
			`.${jie}__area.${jie}__area_crop .zodit-image-editor__sizes`
		) as HTMLElement;
