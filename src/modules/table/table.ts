...
import type { ICreate, IDictionary, IZodit } from 'zodit/types';
...
import { debounce } from 'zodit/core/decorators';
...
export class Table extends ViewComponent<IZodit> {
	/** @override */
	className(): string {
		return 'Table';
	}
	...
	private recalculateStyles(): void {
		...
		style.innerHTML = selectors.length
			? selectors.join(',') +
			  `{${(this.zodit.options as any).table.selectionCellStyle}}`
			: '';
	}
	 ...
	 */
	static mergeSelected(table: HTMLTableElement, zodit: IZodit): void {
		const html: string[] = [],
		...
		if (bound && (bound[0][0] - bound[1][0] || bound[0][1] - bound[1][1])) {
					...
					if (i >= bound[0][0] && i <= bound[1][0]) {
						if (j >= bound[0][1] && j <= bound[1][1]) {
							...
							if (!first) {
								...
								instance(zodit).removeSelection(td);
							}
						}
					}
				}
			...
			if (first) {
				...
				instance(zodit).addSelection(first);
	...
	/**
	 * Divides all selected by `zodit_focused_cell` class table cell in 2 parts vertical. Those division into 2 columns
	 */
	static splitHorizontal(table: HTMLTableElement, zodit: IZodit): void {
		let coord: number[],
			td: HTMLTableCellElement,
		...
		Table.getSelectedCellsByTable(table).forEach(
			(cell: HTMLTableCellElement) => {
				td = zodit.createInside.element('td');
				td.appendChild(zodit.createInside.element('br'));
				tr = zodit.createInside.element('tr');
				...
				Table.unmark(__marked);
				instance(zodit).removeSelection(cell);
			}
		);
	 ...
	 */
	static splitVertical(table: HTMLTableElement, zodit: IZodit): void {
...
const instance = (j: IZodit): Table => j.getInstance<Table>('Table', j.o);
