...
import {
	...
	send,
	sendFiles
} from 'zodit/modules/uploader/helpers';

import './config';

export class Uploader extends ViewComponent implements IUploader {
	override readonly zodit!: IViewBased;
	 ...
	 * Zodit.editors.someidfoeditor.uploader.bind($form[0], function (files) {
	 *     var i;
	...
	bind(
		...
		const onFinally = (): void => {
			form.classList.remove('zodit_drag_hover');
		};
		...
		if (self.j && (self.j as IZodit).editor !== form) {
		...
		self.j.e
			...
			.on(form, 'dragover', (event: DragEvent) => {
				if (
					...
					form.classList.add('zodit_drag_hover');
	...
	constructor(editor: IViewBased, options?: IUploaderOptions<Uploader>) {
		...
		this.options = ConfigProto(
			...
			ConfigProto(
				...
				isZoditObject(editor) ? editor.o.uploader : {}
			)
