...
import { UIButton, UIButtonState } from 'zodit/core/ui/button';
import { autobind, component, watch } from 'zodit/core/decorators';
import { Dom } from 'zodit/core/dom';
import { Popup } from 'zodit/core/ui/popup/popup';
...
import { findControlType } from 'zodit/core/ui/helpers/get-control-type';
	...
	implements IToolbarButton
{
	...
	private calculateActivatedStatus(tc?: ToolbarCollection): boolean {
		if (isZoditObject(this.j) && !this.j.editorIsActive) {
			return false;
		}
	...
	constructor(
		...
		// Prevent lost focus
		zodit.e.on([this.button, this.trigger], 'mousedown', (e: MouseEvent) =>
			e.preventDefault()
		);
	...
	private openControlList(control: IControlTypeStrongList): void {
		const controls: Controls = this.zodit.options.controls ?? {},
