...
> A prerequisite for the module to work is the server side. Zodit offers its own implementation in php [zodit-connectors](https://github.com/techzolab/zodit-connectors). The module will not work without the server side.

You can write the backend yourself, it must support [API](https://github.com/techzolab/zodit-connectors#api)
...
```json
{
	...
	"data": {
		"sources": [
			{
				"baseurl": "https://techzolab.net/zodit/finder/files/",
				...
				return {
					...
					data: {
						sources: [
							{
								baseurl:
									'https://techzolab.net/zodit/finder/files/',
								path: '/folder/somepath/',
								files: resp,
								name: 'default'
							}
...
-   filebrowser.ajax.headers={} An object of additional header key/value pairs toWYSIWYG
    ...
    image. By default, the uses {@link Zodit.defaultOptions.filebrowser.ajax|filebrowser.ajax} with argument
    action=create
-   filebrowser.crop Settings for AJAX connections to the server to crop image.
    By default, the uses {@link Zodit.defaultOptions.filebrowser.ajax|filebrowser.ajax} with argument
    action=create
-   filebrowser.c Settings for AJAX connections to the server to create
    the category . By default, the uses {@link Zodit.defaultOptions.filebrowser.ajax|filebrowser.ajax}
    with argument `action=create`
-   filebrowser.move Settings for AJAX connections to the server for the moving
    image or category . By default uses {@link Zodit.defaultOptions.filebrowser.ajax|filebrowser.ajax}
    with argument `action=move`
-   filebrowser.remove Settings for AJAX connections to the server toWYSIWYG
    delete the image or category . By default uses {@link Zodit.defaultOptions.filebrowser.ajax|filebrowser.ajax}
    ...
    By default uses {@link Zodit.defaultOptions.filebrowser.ajax|filebrowser.ajax}
...
-   filebrowser.items Settings for AJAX connections to the server to download
    ...
    {@link Zodit.defaultOptions.filebrowser.ajax|filebrowser.ajax} with argument action=items
