...
describe('Zodit FileBrowser Tests', function () {
	describe('Constructor/Destructor', function () {
		describe('Without Zodit', function () {
			it('Should create dialog and load files', function () {
				const filebrowser = new Zodit.modules.FileBrowser({
					ajax: {
				...
				expect(
					document.querySelectorAll('.zodit-dialog').length
				).equals(1);
		...
		it('Should create dialog and load files', function () {
			...
			expect(
				editor.ownerDocument.querySelectorAll('.zodit-dialog').length
			).equals(1);
						...
						expect(
							files.querySelector(
								'.' +
									...
									' img[data-src="https://techzolab.net/zodit/files/images.jpg"]'
							)
						).is.not.null;
		...
		describe('Use GET method instead POST', function () {
			it('Should add params into url instead body', function (done) {
				...
				filebrowser
					.open(function () {})
					.then(function () {
						Zodit.modules.Ajax.log.forEach(function (req) {
					...
					.then(function () {
						Zodit.modules.Ajax.log.forEach(function (req) {
							expect(req.url).equals(
								'https://techzolab.net/zodit/connector/index.php'
							);
						});
					...
					.then(function () {
						expect(
							filebrowser.dialog.dialogbox_header.querySelectorAll(
								'.zodit-toolbar-button,.zodit-toolbar-content'
							).length
						).equals(9);
				...
				filebrowser
					.open(function (data) {
						...
						expect(data.files[0]).equals(
							'https://techzolab.net/zodit/files/test.txt'
						);
					})
					.then(function () {
					...
					.then(function () {
						...
						editor.async.requestIdleCallback(() => {
							expect(editor.value).equals(
								'<p><img src="https://techzolab.net/zodit/files/images.jpg"></p>'
							);
			...
			it('Should create A element in editor', function (done) {
				...
				filebrowser
					...
					.then(function () {
						...
						expect(editor.value).equals(
							'<p><a href="https://techzolab.net/zodit/files/test.txt">https://techzolab.net/zodit/files/test.txt</a></p>'
						);
						simulateEvent('drop', window);
			...
			it('Should insert IMG element in editor in the selected before place', function (done) {
				...
				filebrowser
					...
					.then(function () {
						...
						expect(editor.value).equals(
							'<p>Some text</p><p>Another<img src="https://techzolab.net/zodit/files/ibanez-s520-443140.jpg" width="300px"> text</p><p>Another some text</p>'
						);
			...
			it('Should insert A element in editor in the selected before place', function (done) {
				...
				filebrowser
					...
					.then(function () {
						...
						expect(editor.value).equals(
							'<p>Some text</p><p>Another<a href="https://techzolab.net/zodit/files/test.txt" title="https://techzolab.net/zodit/files/test.txt">https://techzolab.net/zodit/files/test.txt</a> text</p><p>Another some text</p>'
						);
				...
				it('Should create IMG element', function (done) {
					...
					editor.events.on('filesWereUploaded', function () {
						expect(sortAttributes(editor.value)).equals(
							'<p><img src="https://techzolab.net/zodit/files/test.png" style="width:300px"></p>'
						);
						done();
					});
				...
				filebrowser
					...
					.then(function () {
						...
						const item = files.querySelector(
							...
							pos = Zodit.modules.Helpers.position(item);
								...
								function () {
									...
									const previewsButtons =
										dialog.querySelectorAll(
											'.zodit-filebrowser-preview__navigation'
										);
