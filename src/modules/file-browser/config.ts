...
import { humanSizeToBytes, isArray, isString } from 'zodit/core/helpers';
import { UIFileInput } from 'zodit/core/ui';
...
Config.prototype.filebrowser = {
	...
	getThumbTemplate(
		...
		return `<a
			data-zodit-filebrowser-item="true"
...
Config.prototype.controls.filebrowser = {
	...
	filter: {
		...
		getContent: (filebrowser: IFileBrowser, _, b): HTMLElement => {
			const oldInput = b.container.querySelector('.zodit-input');
	...
	sort: {
		...
		getContent: (fb: IFileBrowser): HTMLElement => {
			const select: HTMLSelectElement = fb.c.fromHTML(
				'<select class="zodit-input zodit-select">' +
