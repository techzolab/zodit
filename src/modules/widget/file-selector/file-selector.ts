...
import type { IFileBrowserCallBackData, IZodit } from 'zodit/types';
import { isFunction, $$, attr, val } from 'zodit/core/helpers';
import { Dom } from 'zodit/core/dom';
import { TabOption, TabsWidget } from '../tabs/tabs';
import { UIBlock, UIForm, UIInput, UIButton } from 'zodit/core/ui';
...
interface ImageSelectorCallbacks {
	 ...
	 */
	url?: (this: IZodit, url: string, alt: string) => void;
	 ...
	 */
	upload?: ((this: IZodit, data: IFileBrowserCallBackData) => void) | true;
}
 ...
 * @example
 * ```javascript
 * let widget = new Zodit.modules.Widget(editor);
 *
...
export const FileSelectorWidget = (
	...
	if (
		...
		const dragBox = editor.c.fromHTML(
			'<div class="zodit-drag-and-drop__file-box">' +
				`<strong>${editor.i18n(
