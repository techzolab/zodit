...
import { KEY_ESC } from 'zodit/core/constants';
import {
	$$,
	asArray,
	assert,
...
import { ViewWithToolbar } from 'zodit/core/view/view-with-toolbar';
import { Dom } from 'zodit/core/dom';
import { STATUSES } from 'zodit/core/component';
import { eventEmitter, pluginSystem } from 'zodit/core/global';
import { component, autobind, hook } from 'zodit/core/decorators';
import { View } from 'zodit/core/view/view';
...
export class Dialog extends ViewWithToolbar implements IDialog {
	 ...
	 * dialog.setFooter([
	 *  $('<a class="zodit-button">OK</a>').click(function () {
	 *      alert($('someText').val())
	 *      dialog.close();
	 *  })
	 * ]);
	...
	getMaxZIndexDialog(): IDialog {
		...
		$$('.zodit-dialog', this.destination).forEach((dialog: HTMLElement) => {
	...
	setMaxZIndex(): void {
		...
		$$('.zodit-dialog', this.destination).forEach(dialog => {
			zIndex = parseInt(css(dialog, 'zIndex') as string, 10);
	 ...
	 * var dialog = new Zodit.modules.Dialog();
	 * dialog.open('Hello world!', 'Title');
	 * var $close = dialog.create.fromHTML('<a href="#" style="float:left;" class="zodit-button">
	 *     <i class="icon icon-check"></i>&nbsp;' + Zodit.prototype.i18n('Ok') + '</a>');
	...
	close(e?: MouseEvent): this {
		...
		this.e?.fire(this.ow, 'zoditCloseDialog');
	...
	constructor(options?: Partial<IDialogOptions>) {
		...
		self.container = this.c.fromHTML(
			`<div style="z-index:${self.o.zIndex}" class="zodit zodit-dialog ${
				this.componentName
			}">
