	...
	describe('Image properties dialog', function () {
		...
		describe('Change border radius', function () {
			it('should change image border radius', function () {
				...
				simulateEvent(
					'click',
					0,
					dialog.querySelectorAll('.zodit-tabs__buttons button')[1]
				);

				const tab = dialog.querySelector('.zodit-tab.zodit-tab_active');
				...
				it('should change image horizontal align', function () {
					...
					simulateEvent(
						...
						dialog.querySelectorAll(
							'.zodit-tabs__buttons button'
						)[1]
					);

					const tab = dialog.querySelector(
						'.zodit-tab.zodit-tab_active'
					);
					expect(tab).is.not.null;
			...
			describe('Change marginTop with unlock', function () {
				it('should change only marginTop', function () {
					...
					const {
						...
						marginRight,
						lockMargin
					} = Zodit.modules.Helpers.refs(dialog);
		...
		describe('Change link', function () {
			it('should change image wrapper', function () {
				...
				expect(sortAttributes(editor.value)).equals(
					'<p><a href="https://techzolab.net/"><img src="tests/artio.jpg" style="height:100px;width:100px"></a></p>'
				);
			});
			...
			describe('open link in new tab', function () {
				it('should change image wrapper with target="_blank"', function () {
					...
					expect(sortAttributes(editor.value)).equals(
						'<p><a href="https://techzolab.net/" target="_blank"><img src="tests/artio.jpg" style="height:100px;width:100px"></a></p>'
					);
				});
			});
		...
		describe('Show filebrowser buttons and edit image button', function () {
			...
			describe('Uploader and filebrowser settings set', function () {
				...
				it('should not show buttons', function () {
					const editor = Zodit.make(appendTestArea(), settings);
				...
				describe('Click on edit button', function () {
					describe('When photo it is not my', function () {
						it('should open image editor', function (done) {
							...
							editor.value =
								'<img src="https://techzolab.net/zodit/files/artio.jpg"/>';
							...
							expect(
								...
								'https://techzolab.net/zodit/files/artio.jpg'
							);
		...
		describe('In relative object', function () {
			it('should be under image', function () {
				...
				div.innerHTML =
					...
					'            <textarea id="text__area0"> <img src="https://techzolab.net/zodit/files/artio.jpg" style="border:1px solid red;width:100px;height:100px;"/><br/><br/><br/><br/><br/><br/></textarea>\n' +
					'        </div>\n' +
					'    </div>';

				document.body.appendChild(div);
				const editor = new Zodit('#text__area0', {
					history: {
						timeout: 0
					}
				});
				...
				const positionPopup = Zodit.modules.Helpers.position(popup);
				const positionImg = Zodit.modules.Helpers.position(
					editor.editor.querySelector('img')
				);
