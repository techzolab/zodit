 ...
 */
describe('Zodit Editor Tests', function () {
	describe('Constructor', function () {
		it('Constructor Zodit must be in global scope', function () {
			expect(window.Zodit).is.a('function');
		});

		it('Constructor Zodit should have version and esnext field', function () {
			expect(/\d+\.\d+\.\d+/.test(Zodit.prototype.getVersion())).is.true;
			expect(typeof Zodit.esNext).eq('boolean');
		});

		it('Constructor default should not be in global scope', function () {
			expect(window.default).is.not.a('function');
			expect(window.default).does.not.equal(window.Zodit);
		});
		...
		describe('First argument', function () {
			...
			describe('HTMLTextAreaElement', function () {
				it('Should be instance of HTMLElement', function () {
					...
					const editor2 = getZodit(undefined, area);
			...
			describe('HTMLDivElement', function () {
				it('Should be instance of HTMLElement', function () {
					...
					const editor3 = getZodit(undefined, div);
			...
			describe('Found element', function () {
				it('Should be instance of HTMLElement', function () {
					...
					const editor3 = getZodit(undefined, found);
			...
			describe('Double apply to one element', function () {
				it('Should create only one instance', function () {
					...
					const editor4 = getZodit(undefined, div);
		...
		describe('Source textarea', () => {
			it('should have component properties with Zodit instance', function () {
				const area = appendTestArea();
			...
			describe('After destruct', () => {
				it('should not have component properties with Zodit instance', function () {
					const area = appendTestArea();
		...
		describe('Options', function () {
			...
			describe('Set nested array', function () {
				it('Should create editor with merged default array and set array', function () {
					Zodit.defaultOptions.someArray = {
						data: [1, 2, 3, 4]
					};
					const editor = getZodit({
						someArray: {
				...
				describe('Set nested array like Zodit.atom', function () {
			...
			describe('Set nested object', function () {
				it('Should create editor with merged default object and set object', function () {
					Zodit.defaultOptions.someObject = {
						data: {
							left: 10,
							right: 20
						}
				...
				describe('Set nested object like Zodit.atom', function () {
			...
			describe('Statusbar', function () {
				describe('Hide', function () {
					it('should not show statusbar', function () {
						...
						expect(
							editor.container
								.querySelector('.zodit-status-bar')
								.classList.contains('zodit_hidden')
						).is.true;
		...
		describe('Check preset', function () {
			it('Should set option by preset', function () {
				...
				Zodit.defaultOptions.presets.custom = { readonly: true };
				const editor3 = getZodit({
					preset: 'custom'
				});
				expect(editor3.options.readonly).is.true;

				const editor4 = getZodit({
					preset: 'inline',
					inline: false
				});
	...
	describe('Editors stack', function () {
		it('Zodit.instances should contain all instances of Zodit', function () {
			const editor = getZodit(
				undefined,
				appendTestArea('textarea_editor')
			);
			expect(Zodit.instances.textarea_editor).equals(editor);
		});

		it('Zodit.instances should not contain editor after destruct', function () {
			const editor = getZodit(
				undefined,
				appendTestArea('textarea_editor')
			);
			editor.destruct();
			expect(Zodit.instances.textarea_editor).to.be.an('undefined');
		});
	});
	...
	describe('Set/Get', function () {
		...
		it('Set wrong element value', function () {
			const area = appendTestArea(),
				editor = getZodit(undefined, area);
		...
		describe('Synchronization', function () {
			...
			it('Check synchronization between editor and element when was pressed button', function () {
				...
				simulateEvent('keydown', Zodit.KEY_ENTER, editor.editor);
				expect(editor.value).equals(editor.getElementValue());
			});
		});
		...
		describe('Save selection stuf', function () {
			describe('Set false in getEditorValue method', function () {
				it('Should return HTML with selections markers', function () {
					...
					expect(
						/<span[^>]+id="zodit-selection_marker_[^>]+><\/span>/.test(
			...
			describe('Get filebrowser and uploader property from editor', function () {
				describe('FileBrowser', function () {
					it('should create instance of Filebrowser  only one time and in lazy mode', function () {
						...
						editor.components.forEach(function (cmp) {
							expect(
								cmp instanceof Zodit.modules.FileBrowser
							).is.false;
						});

						const filebrowser = editor.filebrowser;
						expect(filebrowser instanceof Zodit.modules.FileBrowser)
							.is.true;

						let instanceCount = 0;

						editor.components.forEach(function (cmp) {
							if (cmp instanceof Zodit.modules.FileBrowser) {
								instanceCount += 1;
				...
				describe('Uploader', function () {
					it('should create instance of Uploader  only one time and in lazy mode', function () {
						...
						editor.components.forEach(function (cmp) {
							expect(
								cmp instanceof Zodit.modules.Uploader
							).is.false;
						});

						const uploader = editor.uploader;
						expect(uploader instanceof Zodit.modules.Uploader).is
							.true;

						let instanceCount = 0;

						editor.components.forEach(function (cmp) {
							if (cmp instanceof Zodit.modules.Uploader) {
								instanceCount += 1;
		...
		describe('Method waitForReady', () => {
			describe('Sync init', () => {
				it('Should return resolved promise', done => {
					...
					expect(zodit.isReady).is.true;

					zodit.waitForReady().then(j => {
						expect(zodit).eq(j);
						expect(zodit.isReady).is.true;
						done();
					});
			...
			describe('Composition event', () => {
				it('should handled normal', () => {
					...
					zodit.value = '<p>test</p>';

					simulateEvent('compositionend', zodit.editor);
					expect(zodit.value).eq('<p>test</p>');
				});
			});
