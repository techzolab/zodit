 ...
 */
describe('Commands Zodit Editor Tests', function () {
	describe('Command "formatBlock"', function () {
		...
		describe('Exec formatBlock for one inline element', function () {
			it('Should wrap this element and all nearest inline element in block', function () {
				...
				zodit.value = 'stop <span>post</span> ice';
				const range = zodit.ed.createRange();
				range.setStart(zodit.editor.firstChild, 0);
				range.setEnd(zodit.editor.firstChild, 2);
				zodit.s.selectRange(range);

				zodit.execCommand('formatBlock', false, 'h1');
