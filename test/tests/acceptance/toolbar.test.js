...
describe('Toolbar', function () {
	it('Should have buttons', function () {
		...
		expect(
			editor.toolbar.container.querySelectorAll('.zodit-toolbar-button')
				.length
		).equals(39);
	});
	...
	describe('Custom buttons', function () {
		it('should create normal button in toolbar', function () {
			const editor = getZodit({
				...
				buttons: [
					'image',
					{
						name: 'alert_some',
						iconURL:
							'https://techzolab.net/zodit/build/images/icons/045-copy.png',
						exec: function () {
							alert('test');
						}
					}
			...
			btns.forEach(function (btn) {
				const icon = btn.querySelector('.zodit-icon');
	...
	describe('Custom icons', function () {
		describe('By default for unknown icons', function () {
			it('should create button as text mode', function () {
				...
				expect(
					...
					'<span class="zodit-toolbar-button__icon"></span><span class="zodit-toolbar-button__text">rect</span>'
				);
			});
		});
		...
		describe('Use getIcon event', function () {
			it('should create buttons with custom icons', function () {
				...
				expect(
					...
					'<span class="zodit-toolbar-button__icon">' +
						'<i class="fa fa-image fa-xs zodit-icon_image zodit-icon" style="font-size:14px"></i>' +
						'</span><span class="zodit-toolbar-button__text"></span>'
				);
				...
				expect(
					...
					'<span class="zodit-toolbar-button__icon">' +
						'<i class="fa fa-rotate-right fa-xs zodit-icon_redo zodit-icon" style="font-size:14px"></i>' +
						'</span><span class="zodit-toolbar-button__text"></span>'
				);
				...
				expect(
					...
					'<span class="zodit-toolbar-button__icon">' +
						'<i class="fa fa-question fa-xs zodit-icon_about zodit-icon" style="font-size:14px"></i>' +
						'</span><span class="zodit-toolbar-button__text"></span>'
				);
			});
		});
		...
		const rect =
			...
			rectResult =
				'<span class="zodit-toolbar-button__icon">' +
				'<svg class="zodit-icon_rect zodit-icon" height="110" width="400"><rect height="100" width="300"></rect></svg>' +
				'</span><span class="zodit-toolbar-button__text"></span>';
		...
		describe('Use Icon module', function () {
			it('should create buttons with custom icons', function () {
				Zodit.modules.Icon.set('rect', rect);
				const editor = getZodit({
					toolbarAdaptive: false,
					buttons: [
						{
		...
		describe('Use svg in icon', function () {
			it('should create buttons with custom icons', function () {
				...
				expect(
					sortAttributes(getButton('rect', editor).innerHTML)
				).equals(rectResult.replace('zodit-icon_rect ', ''));
			});
		});
	});
	...
	describe('Set toolbar options to false', function () {
		it('Should hide toolbar', function () {
			...
			expect(null).equals(
				editor.container.querySelector('.zodit_toolbar')
			);
		});
	});
	...
	describe('Set toolbar options to css selector', function () {
		it('Should render toolbar in different container', function () {
			...
			const toolbar = document.querySelector(
				'.zodit-toolbar-editor-collection'
			);
		...
		describe('After enable Fullsize mode', function () {
			it('Should render toolbar in default container', function () {
				const div = appendTestDiv(),
					editor = Zodit.make(appendTestArea(), {
						toolbar: div
					});
	...
	describe('Popups', function () {
		...
		describe('Double open list or popup', () => {
			it('should close previous popup', () => {
				...
				clickButton('image', zodit);
				expect(getOpenedPopup(zodit)).is.not.null;
				clickButton('image', zodit);
				expect(getOpenedPopup(zodit)).is.null;
			});
			...
			describe('and click another', () => {
				it('should close previous popup and show own', () => {
					const zodit = getZodit();
					clickButton('image', zodit);
					const popup = getOpenedPopup(zodit);
					expect(popup).is.not.null;
					clickButton('video', zodit);
					expect(getOpenedPopup(zodit).innerText !== popup.innerText)
						.is.true;
				});
			});
			...
			describe('and click outside', () => {
				it('should close previous popup', () => {
					...
					clickButton('image', zodit);
					expect(getOpenedPopup(zodit)).is.not.null;
					simulateEvent('mousedown', zodit.editor);
					expect(getOpenedPopup(zodit)).is.null;
				});
			});
		});
		...
		it('Open video dialog and insert video by url from youtube.', function () {
			...
			simulateEvent('submit', popup.querySelector('.zodit-ui-form'));

			expect(
				popup.querySelectorAll(
					'.zodit-ui-input.zodit-ui-input_has-error_true'
				).length
			).equals(1);
		...
		describe('Create table', function () {
			describe('Mouse move', function () {
				it('Should highlight cells in table-creator', function () {
					...
					simulateEvent(
						'mousemove',
						list.querySelectorAll('.zodit-form__container span')[14]
					);

					expect(
						list.querySelectorAll(
							'.zodit-form__container span.zodit_hovered'
						).length
					).equals(10);
				});
	...
	describe('Buttons', function () {
		...
		describe('Text mode', function () {
			...
			it("Should add zodit_text_icons class to editor's container", function () {
				const editor = getZodit({
					textIcons: true
				});
		...
		describe('Set empty Zodit.atom', function () {
		...
		it('Add own button', function () {
			const editor = getZodit({
				disablePlugins: ['mobile'],
				buttons: Zodit.defaultOptions.buttons.concat([
					{
						name: 'insertDate',
						iconURL: 'http://techzolab.net/opensource/zodit/images/logo.png',
			...
			describe('For list', function () {
				describe('enable', function () {
					it('Should enable buttons which can be used in this mode', function () {
						...
						expect(
							list.querySelectorAll('.zodit_disabled').length
						).equals(0);
					});
				});
				...
				describe('disable', function () {
					it('Should disable buttons which can not be used in that mode', function () {
						...
						const list = btn.querySelector('.zodit_toolbar_list');
						expect(list).is.null;
					});
				});
			});
			...
			it('Should add modification to button', function () {
				...
				expect(
					getButton(
						...
						'zodit-toolbar-button_size_small'
					)
				).is.true;
			});
		...
		it('Full size button', function () {
			...
			while (node && node.nodeType !== Node.DOCUMENT_NODE) {
				expect(
					node.classList.contains('zodit_fullsize-box_true')
				).equals(true);
	...
	describe('Commands', function () {
		it('Click on Source button should change current mode', function () {
			...
			expect(editor.getMode()).equals(Zodit.MODE_SOURCE);
		});
			...
			it('should hide toolbar buttons', function () {
				...
				expect(3).equals(
					dialog.querySelectorAll(
						'.zodit-dialog__header .zodit-dialog__header-title button,' +
							'.zodit-dialog__header .zodit-dialog__header-title select'
					).length
				);
			});
		});
	...
	describe('One toolbar for several editors', function () {
		it('Should create one Zodit instance but with several edit places', function () {
			const toolbarBox = appendTestDiv(),
				...
				editor = Zodit.make(firstEditPlace);
			...
			expect(Zodit.instances.firstEditPlace).equals(editor);
			expect(Zodit.instances.secondEditPlace).equals(undefined);
			expect(Zodit.instances.thirdEditPlace).equals(undefined);
		...
		describe('For all instances you can set self options', function () {
			it('Should change options for all instances', function () {
				...
				const editPlaces =
					editor.ownerDocument.querySelectorAll('.zodit-wysiwyg');
