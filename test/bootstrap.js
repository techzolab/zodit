...
const oldI18n = Zodit.prototype.i18n,
	oldAjaxSender = Zodit.modules.Ajax.prototype.send,
...
function mockAjax() {
	if (typeof window.chai !== 'undefined') {
		let temp = {};
		Zodit.modules.Ajax.prototype.send = function () {
			const ajax = this;
				...
				switch (action) {
					...
					case 'fileUpload': {
						...
						resolve({
							...
							data: {
								baseurl: 'https://techzolab.net/zodit/files/',
								messages: [],
								files: [file],
								...
								sources: [
									{
										name: 'default',
										baseurl:
											'https://techzolab.net/zodit/files/',
										path: '',
										files: files(path)
									}
								],
					...
					case 'getLocalFileByUrl':
						switch (ajax.options.data.url) {
							...
							case location.protocol +
								'//' +
								location.host +
								'/zodit/test/tests/artio.jpg':
							case 'https://techzolab.net/zodit/files/th.jpg':
								resolve({
									success: true,
...
function unmockAjax() {
	Zodit.modules.Ajax.prototype.send = oldAjaxSender;
}
...
Zodit.prototype.i18n = function (key) {
...
Zodit.defaultOptions.events.afterInit = function (editor) {
	editor &&
		editor.container.setAttribute('data-test-case', window.mochaTestName);
};
Zodit.defaultOptions.filebrowser.saveStateInStorage = false;
Zodit.defaultOptions.history.timeout = 0;
Zodit.defaultOptions.defaultTimeout = 0;
Zodit.defaultOptions.filebrowser.defaultTimeout = 0;
Zodit.modules.View.defaultOptions.defaultTimeout = 0;

if (Zodit.defaultOptions.cleanHTML) {
	Zodit.defaultOptions.cleanHTML.timeout = 0;
	Zodit.defaultOptions.cleanHTML.fillEmptyParagraph = false;
}

Zodit.defaultOptions.resizer.forImageChangeAttributes = false;

Zodit.defaultOptions.sourceEditor = 'area';
Zodit.defaultOptions.language = 'en';
Zodit.defaultOptions.iframeCSSLinks.push('/app.css');
Zodit.defaultOptions.iframeStyle +=
	'* {\
...
const stringify = Zodit.ns.Helpers.stringify;
...
function removeStuff() {
	Object.keys(Zodit.instances).forEach(function (key) {
		Zodit.instances[key].destruct();
	});
	...
	Array.from(
		document.querySelectorAll(
			'.zodit.zodit-dialog.zodit-dialog_active_true'
		)
	).forEach(function (dialog) {
		simulateEvent('close_dialog', 0, dialog);
	});

	Zodit.modules.Ajax.log.length = 0;
 ...
 * @return {Zodit}
 */
function getZodit(options, element) {
	const editor = Zodit.make(element || appendTestArea(), options);

	window.scrollTo(
		0,
		Zodit.modules.Helpers.offset(editor.container, editor, editor.od).top
	);
		...
		.map(function (elm) {
			...
			if (/rgb\(/.test(keyValue[1])) {
				keyValue[1] = keyValue[1].replace(
					/rgb\([^)]+\)/,
					function (match) {
						return Zodit.modules.Helpers.normalizeColor(match);
					}
				);
			}
			...
			if (/font-family/i.test(keyValue[0])) {
				keyValue[1] = keyValue[1]
					.split(',')
					.map(Zodit.modules.Helpers.trim)
					.map(function (value) {
...
function sortAttributes(html) {
	...
	return html
		.replace(/&nbsp;/g, ' ')
		.replace(Zodit.INVISIBLE_SPACE_REG_EXP(), '');
}
 ...
 * @param {string|number|HTMLElement|Zodit} keyCodeOrElement
 * @param {HTMLElement|Zodit} [element]
...
function simulateEvent(type, keyCodeOrElement, elementOrApplyOpt, applyOpt) {
	...
	if (applyOpt) {
		...
		const pos = Zodit.modules.Helpers.position(element);
...
function getOpenedDialog(editor) {
	const dlgs = editor.ownerDocument.querySelectorAll('.zodit-dialog');
 ...
 * @param {Zodit|Element} zoditOrElement
 * @param {string} [role]
 * @param {boolean} [last]
 * @returns {HTMLElement|null}
 */
function getButton(buttonName, zoditOrElement, role, last) {
	const elm = zoditOrElement.container || zoditOrElement;

	return (
		elm.querySelector(
			'.zodit-toolbar-button.zodit-toolbar-button_' +
				buttonName +
				(last ? ':last-child' : '') +
				' [role="' +
				(role || 'button') +
		...
		elm.querySelector(
			'.zodit-ui-button.zodit-ui-button_' +
				buttonName +
				(last ? ':last-child' : '') +
				'[role="' +
				(role || 'button') +
 ...
 *
 * @param {string} buttonName
 * @param {Zodit|HTMLElement} zoditOrElement
 * @param {string} [role]
 * @param {boolean} [last]
 */
function clickButton(buttonName, zoditOrElement, role, last) {
	simulateEvent(
		['mousedown', 'mouseup', 'click'],
		getButton(buttonName, zoditOrElement, role, last)
	);
}

function clickTrigger(buttonName, zoditOrElement) {
	clickButton(buttonName, zoditOrElement, 'trigger');
}
...
function setCursorToChar(editor, char = '|') {
	...
	Zodit.modules.Dom.each(editor.editor, function (node) {
...
function replaceCursorToChar(editor, char = '|') {
	...
	editor.s.markers.forEach(m => {
		Zodit.modules.Dom.after(m, editor.createInside.text(char));
	});
