...
config/* Config.prototype.controls.speechRecognize */.D.prototype.controls.speechRecognize = {
    isActive(zodit, _) {
        const api = (0,data_bind/* dataBind */.q)(zodit, 'speech');
        return Boolean(api === null || api === void 0 ? void 0 : api.isEnabled);
    },
    isDisabled(zodit) {
        return !zodit.o.speechRecognize.api;
    },
    exec(zodit, current, { button, control }) {
        const { api: Api, lang, continuous, interimResults, sound } = zodit.o.speechRecognize;
        if (!Api) {
            (0,dialog_alert/* Alert */.b)('Speech recognize API unsupported in your browser');
            return;
        }
        let api = (0,data_bind/* dataBind */.q)(zodit, 'speech');
        if (!api) {
            ...
            api.sound = sound;
            (0,data_bind/* dataBind */.q)(zodit, 'speech', api);
            api.on('pulse', (enable) => {
                button.setMod('pulse', enable);
            });
            api.on('result', (text) => zodit.e.fire('speechRecognizeResult', text));
            api.on('progress', (text) => zodit.e.fire('speechRecognizeProgressResult', text));
    ...
    childTemplate(zodit, key, value) {
        var _a;
        const api = (0,data_bind/* dataBind */.q)(zodit, 'speech'), checked = (_a = api === null || api === void 0 ? void 0 : api[key]) !== null && _a !== void 0 ? _a : zodit.o.speechRecognize[key];
        return `<span class='zodit-speech-recognize__list-item'><input ${checked ? 'checked' : ''} class='zodit-checkbox' type='checkbox'>&nbsp;${value}</span>`;
    },
    mods: {
        stroke: false
    }
...
class SpeechRecognizeNative extends core_plugin/* Plugin */.S {
    ...
    afterInit(zodit) {
        const { commands } = zodit.o.speechRecognize;
        if (commands) {
            ...
            (0,utils/* keys */.XP)(commands, false).forEach(words => {
                ...
                keys.forEach(key => {
                    ...
                    const translatedKeys = zodit.i18n(key);
